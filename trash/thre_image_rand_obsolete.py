...
ra_bound = np.linspace(np.deg2rad(ra_min), np.deg2rad(ra_max), n_tile_ra+1)

# start uniform random sampling
while len(rand_ind) < n_rand:
	ind_size = len(dc2_ind)
	ind_rand = rand.randint(0,ind_size, n_per_tile)
	for index in ind_rand:
		i = np.digitize(dc2_truth_shear[dc2_ind[index]]['ra'], ra_bound)-1
		if(i > n_tile_ra-1 or i < 0):  # discard obj exceeding the bounds
			continue
		j = np.digitize(dc2_truth_shear[dc2_ind[index]]['dec'], dec_bound)-1
...
# output to csv file
with open('ra_rand'+'_man_1.csv', 'w') as ra_rand_w:
	writer = csv.writer(ra_rand_w)
	writer.writerow(ra_rand)

with open('dec_rand'+'_man_1.csv', 'w') as dec_rand_w:
