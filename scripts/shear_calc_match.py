...
for i in range(20,26):
    ...
    if i == 20:
        ...
        with open('data/s1_match.csv', 'w') as fs1:
            writer = csv.writer(fs1)
            writer.writerow(s1)
        del(s1)

        with open('data/s2_match.csv', 'w') as fs2:
    ...
    with open('data/s1_thre_match_'+ str(mag_threshold) +'.csv', 'w') as fs1_thre:
        writer = csv.writer(fs1_thre)
        writer.writerow(s1_thre)
        
    del(s1_thre)

    with open('data/s2_thre_match_'+ str(mag_threshold) +'.csv', 'w') as fs2_thre:
