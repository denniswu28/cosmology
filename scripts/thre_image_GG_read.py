...
gg_2 = treecorr.GGCorrelation(min_sep=1, max_sep=100, nbins=20, sep_units='arcmin', var_method='jackknife')
gg_2.read("data/gg_corr_all.fits", file_type="FITS")
...
plt.errorbar(np.exp(gg_2.meanlogr), gg_2.xip, yerr=gg_2.varxip, linestyle="",marker="*", color='black', label = r'Truth full $\xi_+ (\theta)$')
plt.errorbar(np.exp(gg_2.meanlogr), gg_2.xim, yerr=gg_2.varxim, linestyle="",marker=".", color='black', label = r'Truth full $\xi_- (\theta)$')
plt.xscale('log')
plt.yscale('log')
plt.ylim(1e-7,1e-3)
