...
for i in range(20,26):
    ...
    with open('data/ra_rand_match_gal_'+ str(mag_threshold) +'.csv', newline='') as rafile:
        ra_rand = np.float_(list(csv.reader(rafile, delimiter=',')))
    ra_rand = ra_rand[0]

    with open('data/dec_rand_match_gal_'+ str(mag_threshold) +'.csv', newline='') as decfile:
    ...
    dd = treecorr.NNCorrelation(min_sep=1, max_sep=100, nbins=20, sep_units='arcmin', var_method='jackknife')
    dr = treecorr.NNCorrelation(min_sep=1, max_sep=100, nbins=20, sep_units='arcmin')
    rr = treecorr.NNCorrelation(min_sep=1, max_sep=100, nbins=20, sep_units='arcmin')
    ...
    dd.write("data/nn_corr_matched"+ str(mag_threshold) +".fits")
